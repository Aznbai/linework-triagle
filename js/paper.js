...
var paper = new function() {
  var Base = new function() {
      ...
      function field(name, val, dontCheck, generics) {
        ...
        if ((dontCheck || val !== undefined && has.call(src, name)) && (!preserve || !prev)) {
          if (func) {
            ...
            if (beans && val.length == 0 && (bean = name.match(/^(get|is)(([A-Z])(.*))$/)))
    ...
    function each(obj, iter, bind, asArray) {
      try {
        if (obj)
          (asArray || asArray === undefined && isArray(obj) ? forEach : forIn).call(obj, iterator(iter),
            bind = bind || obj);
      } catch (e) {
  ...
  this.Base = Base.inject({
    ...
    statics: {
      read: function(list, start, length) {
        ...
        var obj = list[start];
        if (obj instanceof this || this.prototype._readNull && obj == null && length <= 1)
          return obj;
  ...
  var Matrix = this.Matrix = Base.extend({
    ...
    toString: function() {
      ...
      return '[[' + [format(this._a), format(this._b),
        format(this._tx)
      ].join(', ') + '], [' + [format(this._c), format(this._d),
        format(this._ty)
      ].join(', ') + ']]';
    },
    getValues: function() {
  ...
  var Item = this.Item = Base.extend({
    ...
    getPosition: function() {
      var pos = this._position || (this._position = this.getBounds().getCenter());
    ...
    hitTest: function(point, options, matrix) {
      ...
      return this._children || !(options.guides && !this._guide || options.selected && !this._selected) ? this._hitTest(point, options, matrix) : null;
    },
    ...
    isGroupedWith: function(item) {
      ...
      while (parent) {
        if (parent._parent && (parent instanceof Group || parent instanceof CompoundPath) && item.isDescendant(parent))
          return true;
  ...
  var PlacedItem = this.PlacedItem = Item.extend({
    ...
    _getBounds: function(getter, cacheName, args) {
      ...
      matrix = matrix ? matrix.clone().concatenate(this._matrix) : this._matrix;
  ...
  var PlacedSymbol = this.PlacedSymbol = PlacedItem.extend({
    initialize: function(symbol, matrixOrOffset) {
      ...
      this._matrix = matrixOrOffset !== undefined ? matrixOrOffset instanceof Matrix ? matrixOrOffset : new Matrix().translate(Point.read(arguments, 1)) : new Matrix();
    },
    getSymbol: function() {
      return this._symbol;
    },
  ...
  var Curve = this.Curve = Base.extend({
      ...
      for (var i = 0; i < num; i++) {
        ...
        if (t >= 0 && t < 1 && Curve.evaluate(vals, t, 0).x > point.x) {
          if (t < Numerical.TOLERANCE && Curve.evaluate(
              this.getPrevious().getValues(matrix), 1, 1).y * Curve.evaluate(vals, t, 1).y >= 0)
            continue;
          crossings++;
        }
      }
    ...
    statics: {
      ...
      getValues: function(segment1, segment2, matrix) {
        ...
        return matrix ? matrix._transformCoordinates(coords, 0, coords, 0, 4) : coords;
      },
      ...
      switch (countCrossings(w)) {
        ...
        case 1:
          ...
          if (isFlatEnough(w)) {
            ...
            return [line.vector.getLength(true) <= Numerical.EPSILON ? line.point.x : xAxis.intersect(line).x];
          }
      }
      var p = [
          []
  ...
  var Path = this.Path = PathItem.extend({
    ...
    setFullySelected: function(selected) {
      ...
      for (var i = 0; i < length; i++)
        this._segments[i]._selectionState = selected ? SelectionState.POINT : 0;
      this.setSelected(selected);
    },
    ...
    _hitTest: function(point, options, matrix) {
      ...
      function checkSegment(segment, ends) {
        ...
        for (var j = ends || options.segments ? 0 : 2,
          ...
          if (point.getDistance(coords[j], coords[j + 1]) < tolerance)
            return new HitResult(j == 0 ? 'segment' : 'handle-' + (j == 2 ? 'in' : 'out'),
              that, {
                segment: segment
              });
        }
      ...
      if (options.ends && !options.segments && !this._closed) {
        if (res = checkSegment(this.getFirstSegment(), true) || checkSegment(this.getLastSegment(), true))
          return res;
      ...
      if (!(loc && loc._distance <= radius) && options.fill && this.getFillColor() && this.contains(point, matrix))
        return new HitResult('fill', this);
      if (!loc && options.stroke && radius > 0)
        loc = this.getNearestLocation(point, matrix);
      if (loc && loc._distance <= radius)
        return options.stroke ? new HitResult('stroke', this, {
          location: loc
    ...
    return {
      draw: function(ctx, param) {
        ...
        if (param.compound || param.selection || this._clipMask || fillColor || strokeColor && !hasDash) {
    ...
    function getPenPadding(radius, matrix) {
      ...
      var tx = -Math.atan(b * Math.tan(phi)),
        ...
        x = a * Math.cos(tx) * Math.cos(phi) - b * Math.sin(tx) * Math.sin(phi),
        y = b * Math.sin(ty) * Math.cos(phi) + a * Math.cos(ty) * Math.sin(phi);
    ...
    return {
      ...
      getStrokeBounds: function() {
        ...
        function add(point) {
          bounds = bounds.include(matrix ? matrix.transform(point) : point);
        }
        ...
        function addJoin(segment, join) {
          if (join === 'round' || !segment._handleIn.isZero() && !segment._handleOut.isZero()) {
            bounds = bounds.unite(joinBounds.setCenter(matrix ? matrix.transform(segment._point) : segment._point));
          } else if (join == 'bevel') {
      ...
      getRoughBounds: function() {
        ...
        var bounds = this.getHandleBounds(arguments[0], this.strokeWidth,
          this.getStrokeJoin() == 'miter' ? this.strokeWidth * this.getMiterLimit() : this.strokeWidth);
        if (useCache)
    ...
    var fields = {
      ...
      initialize: function(arg) {
        ...
        if (typeof arg === 'object' && !isArray) {
          if (!type) {
            return arg.red !== undefined ? new RgbColor(arg.red, arg.green, arg.blue, arg.alpha) : arg.gray !== undefined ? new GrayColor(arg.gray, arg.alpha) : arg.lightness !== undefined ? new HslColor(arg.hue, arg.saturation, arg.lightness,
              arg.alpha) : arg.hue !== undefined ? new HsbColor(arg.hue, arg.saturation, arg.brightness,
              arg.alpha) : new RgbColor();
          } else {
      ...
      convert: function(type) {
        var converter;
        return this._colorType == type ? this.clone() : (converter = converters[this._colorType + '-' + type]) ? converter(this) : converters['rgb-' + type](
  ...
  var View = this.View = PaperScopeItem.extend({
    ...
    initialize: function(canvas) {
      ...
      if (canvas instanceof HTMLCanvasElement) {
        ...
        if (PaperScript.hasAttribute(canvas, 'resize')) {
          ...
          size = DomElement.isInvisible(canvas) ? Size.create(parseInt(canvas.getAttribute('width')),
            parseInt(canvas.getAttribute('height'))) : DomElement.getSize(canvas);
        }
    ...
    function mousemove(event) {
      ...
      } else if ((!dragging || onlyMove) && tool.onHandleEvent('mousemove', point, event)) {
        view.draw(true);
        DomEvent.stop(event);
      }
    }
    ...
    onHandleEvent: function(type, pt, event) {
      ...
      switch (type) {
        ...
        case 'mousedrag':
          ...
          break;
        case 'mouseup':
          if ((this._point.x != pt.x || this._point.y != pt.y) && this.updateEvent('mousedrag', pt, this.minDistance,
